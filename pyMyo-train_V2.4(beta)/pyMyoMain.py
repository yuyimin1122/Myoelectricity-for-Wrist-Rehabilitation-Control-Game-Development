...
dirname = r'C:\pyMyo-train_V2.4(beta)'
subjectnum = 0
statuscount = 0
...
class Win(QWidget):
    ...
    def connection(self):
        ...
        # path = dirname + r'\Data\S' + str(subjectnum) + r'_emg_train_pose_data.csv'
        # ed.to_csv(path, mode='a', header=False, index=False)
    ...
    def lowpass(self,data,Fc):
        b, a = signal.butter(2, 2.0*Fc/200, 'lowpass')   
        filtedData = signal.filtfilt(b, a, data)
        return filtedData

    def equationcalculate(self,rawdata,samplerate):
        """
    ...
    def get_emg_data(self):
        ...
        #step 2 滤波去除前两秒数据即滤波器启动时间
        self.emg_max = (self.equationcalculate(raw_emg5[0:8,:],200))[:,400:]
        # for i in range(8):
        #     self.MVC[i] = np.mean(self.emg_max[i,:])
        self.MVC = np.max(self.emg_max)
        print(self.MVC)
        self.emg_max = self.emg_max / self.MVC
        self.emg_min = (self.equationcalculate(raw_emg6[0:8,:],200) / self.MVC)[:,400:]
        self.emg_pose1 = (self.equationcalculate(raw_emg1[0:8,:],200) / self.MVC)[:,400:]
        self.emg_pose2 = (self.equationcalculate(raw_emg2[0:8,:],200) / self.MVC)[:,400:]
        self.emg_pose3 = (self.equationcalculate(raw_emg3[0:8,:],200) / self.MVC)[:,400:]
        self.emg_pose4 = (self.equationcalculate(raw_emg4[0:8,:],200) / self.MVC)[:,400:]

        ed = pd.DataFrame(np.transpose(self.emg_max))
        path = dirname + r'\Data\S' + str(subjectnum) + r'_emg_max_stiffness_filtered_data.csv'
        ed.to_csv(path, mode='w', header=False,index=False)
        ed = pd.DataFrame(np.transpose(self.emg_min))
        path = dirname + r'\Data\S' + str(subjectnum) + r'_emg_min_stiffness_filtered_data.csv'
        ed.to_csv(path, mode='w', header=False,index=False)
        ed = pd.DataFrame(np.transpose(self.emg_pose1))
        path = dirname + r'\Data\S' + str(subjectnum) + r'_emg_pose1_filtered_data.csv'
        ed.to_csv(path, mode='w', header=False,index=False)
        ed = pd.DataFrame(np.transpose(self.emg_pose2))
        path = dirname + r'\Data\S' + str(subjectnum) + r'_emg_pose2_filtered_data.csv'
        ed.to_csv(path, mode='w', header=False,index=False)
        ed = pd.DataFrame(np.transpose(self.emg_pose3))
        path = dirname + r'\Data\S' + str(subjectnum) + r'_emg_pose3_filtered_data.csv'
        ed.to_csv(path, mode='w', header=False,index=False)
        ed = pd.DataFrame(np.transpose(self.emg_pose4))
        path = dirname + r'\Data\S' + str(subjectnum) + r'_emg_pose4_filtered_data.csv'
        ed.to_csv(path, mode='w', header=False,index=False)
    ...
    def get_W(self):
        ...
        self.pose1label = self.get_label(h1)
        self.pose2label = self.get_label(h2)
        self.pose3label = self.get_label(h3)
        self.pose4label = self.get_label(h4)

        W = np.c_[W1,W2,W3,W4]
        return W


    # def get_pose_seed(self,list):
    #     length = len(list[0])
    ...
    def get_label(self,list):
        ...
        for i in range(length):
            label[i] = list[0,i]
        label = self.lowpass(label,1)
        labelmin = np.min(label)
        labelmax = np.max(label)
        labelseed = np.zeros(length)
        for i in range(length):
            labelseed[i] = (label[i] - labelmin)/(labelmax - labelmin)
        return labelseed
    ...
    def get_parameter(self,W):
        ...
        pose1_train = self.pose1label
        pose2_train = self.pose2label
        pose3_train = self.pose3label
        pose4_train = self.pose4label
        ...
        label = np.vstack((Y1_train,Y2_train))

        ed = pd.DataFrame(np.transpose(label))
        path = dirname + r'\Data\S' + str(subjectnum) + r'_emg_label_data.csv'
        ed.to_csv(path, mode='w', header=False,index=False)
        #step 4 训练
        DOF1_X = np.c_[X1_train,X2_train,X3_train,X4_train]
        DOF1_Y = Y1_train.reshape(-1,1)
        lrModel.fit(DOF1_X,DOF1_Y)
        t11 = lrModel.coef_[0][0]
        t12 = lrModel.coef_[0][1]
        t13 = lrModel.coef_[0][2]
        t14 = lrModel.coef_[0][3]
        #DOF1 线性回归
        DOF2_X = np.c_[X1_train,X2_train,X3_train,X4_train]
        DOF2_Y = Y2_train.reshape(-1,1)
        lrModel.fit(DOF2_X,DOF2_Y)
        t21 = lrModel.coef_[0][0]
        t22 = lrModel.coef_[0][1]
        t23 = lrModel.coef_[0][2]
        t24 = lrModel.coef_[0][3]
        ...
        ed = pd.DataFrame(list2,columns=["k_max","k_min","t11","t12","t13","t14","t21","t22","t23","t24","MVC"])        
