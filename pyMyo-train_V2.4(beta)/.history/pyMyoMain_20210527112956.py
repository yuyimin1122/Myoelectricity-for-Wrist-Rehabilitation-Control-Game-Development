...
class Win(QWidget):
    ...
    def get_W(self):
        ...
        W1 = self.normalize(w1,self.emg_pose1)
        W2 = self.normalize(w2,self.emg_pose2)
        W3 = self.normalize(w3,self.emg_pose3)
        W4 = self.normalize(w4,self.emg_pose4)
    ...
    def get_pose_seed(self,list):
        #输入4*n控制信号
        length = len(list[0])
        per_length = int(length/27)
        sin1_seed = np.zeros(per_length*2)
        for i in range(per_length*2):
            sin1_seed[i] = math.sin((i/(per_length*2))*(math.pi/2))
        sin2_seed = np.zeros(per_length*2)
        for i in range(per_length*2):
            sin2_seed[i] = math.sin((i/(per_length*2))*(math.pi/2)+(math.pi/2))
        zero_seed = np.zeros(per_length)
        one_seed = zero_seed + 1 
        onetrail_seed = np.hstack((zero_seed,zero_seed,sin1_seed,one_seed,sin2_seed,zero_seed,zero_seed))
        pose_seed = np.hstack((onetrail_seed,onetrail_seed,onetrail_seed))
        pose_seed = np.hstack((pose_seed , np.zeros(len(list[0])-len(pose_seed))))
        return pose_seed
    ...
    def normalize(self,list1,list2):
        Amax = np.dot(np.transpose(list1),list2).max()
        Amin = np.dot(np.transpose(list1),list2).min()
        normalizedate = list1 /(Amax - Amin)
